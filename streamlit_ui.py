...
from langfuse_dashboard_v4 import LangfuseDashboard, FeedbackCollector
# CHANGED: propagate_attributes replaces passing user_id/session_id to span.update()
...
# Start an initial trace for the session and store it
        ...
        with langfuse_config.langfuse.start_as_current_observation(
            name="dashboard_load"
        ) as span:
            # CHANGED: user_id/session_id moved to propagate_attributes()
...
def clear_conversation(auto=False):
    ...
    with langfuse_config.langfuse.start_as_current_observation(
        ...
        with propagate_attributes(
            user_id=st.session_state.user_id,
            session_id=st.session_state.session_id,
            metadata={"auto_triggered": str(auto)}
        ):
            pass
