...
from configure_langfuse_v4 import configure_langfuse
import time
...
async def get_embedding(text: str, openai_client: AsyncOpenAI, deps: CombinedDeps = None, run_context=None, trace=None) -> List[float]:
    ...
    for attempt in range(max_retries):
        try:
            # Use Langfuse v4 API
            with deps.langfuse_client.start_as_current_observation(name="text_embedding") as span:
                span.update(
                    metadata={
            ...
            if attempt == max_retries - 1:
                ...
                with deps.langfuse_client.start_as_current_observation(name="text_embedding_error") as error_span:
                    error_span.update(
                        metadata={
                            "error": str(e),
                            "attempt": attempt + 1,
...
async def retrieve_from_qdrant(ctx: RunContext[CombinedDeps], user_query: str) -> str:
    ...
    try:
        ...
        with ctx.deps.langfuse_client.start_as_current_observation(name="qdrant_retrieval") as span:
            span.update(
                metadata={
            ...
            with ctx.deps.langfuse_client.start_as_current_observation(name="get_embedding_wrapper") as embedding_wrapper_span:
            ...
            with ctx.deps.langfuse_client.start_as_current_observation(name="standard_vector_search") as search_span:
                search_span.update(
                    metadata={
                        "namespace": "hip_pages",
                        "top_k": 5
            ...
            for year in years_to_check:
                with ctx.deps.langfuse_client.start_as_current_observation(name=f"year_search_{year}") as year_span:
                    year_span.update(
                        metadata={
                            "year": year
                        }
        ...
        if not combined_results:
            with ctx.deps.langfuse_client.start_as_current_observation(name="text_search_fallback") as text_search_span:
...
"""
        ...
        # Format results
        with ctx.deps.langfuse_client.start_as_current_observation(name="format_results") as format_span:
            formatted_chunks = []
...
async def retrieve_from_pinecone(ctx: RunContext[CombinedDeps], user_query: str) -> str:
    ...
    try:
        ...
        # Start a new span for the retrieval operation using Langfuse v4
        with ctx.deps.langfuse_client.start_as_current_observation(name="pinecone_retrieval") as span:
            # Update span with input data (Langfuse v4 way)
            span.update(
                input={
            ...
            with ctx.deps.langfuse_client.start_as_current_observation(name="get_embedding_for_retrieval") as embedding_span:
            ...
            with ctx.deps.langfuse_client.start_as_current_observation(name="index_access") as index_span:
                index = ctx.deps.pinecone_client.Index("forum-pages")
                index_span.update(
            ...
            with ctx.deps.langfuse_client.start_as_current_observation(name="combine_results") as combine_span:
                all_matches = {}
        ...
        # Log the error with tracing using Langfuse v4
        try:
            with ctx.deps.langfuse_client.start_as_current_observation("retrieval_error") as error_span:
                error_span.update(
...
async def evaluate_retrieval(
    ...
    # CHANGED: langfuse_span is now opened inside the OTel span so it stays open
    # for the nested langfuse_generation call on line ~996
    with tracer.start_as_current_span("rag_evaluation") as span:
        ...
        try:
            ...
            with tracer.start_as_current_span("llm_evaluation_call") as eval_span:
                ...
                # CHANGED: create langfuse generation directly from the client, not from langfuse_span
                if ctx.deps.langfuse_client:
                    langfuse_generation = ctx.deps.langfuse_client.start_as_current_observation(
                        name="evaluation_llm_call", as_type="generation", model="gpt-4o-mini"
                    ).__enter__()
